...
get_commit_info()
{
    ...
    forward_commit=$(git log HEAD^..${cur_remote_branch}/HEAD --oneline | grep -B 1 $(git rev-parse --short HEAD) | head -1)
    forward_com_id=$(echo ${forward_commit} | awk '{print $1}')
    echo "forward commit:    ${forward_commit}"
    echo "forward com_id:    ${forward_com_id}"

    # cur
    cur_commit=$(git log --oneline | grep $(git rev-parse --short HEAD))
    cur_com_id=$(echo ${cur_commit} | awk '{print $1}')
    echo "cur commit:        ${cur_commit}"
    echo "cur com_id:        ${cur_com_id}"

    # backward
    backward_commit=$(git log --oneline | grep -A 1 $(git rev-parse --short HEAD) | tail -1)
